...
module.exports = async function (req, res) {
  ...
  if (!API_KEY) return sendJSON(res, 500, { error: 'Thiếu YOUTUBE_API_KEY trong Environment Variables' });

  // Hỗ trợ cả ?channel= và ?channelId=
  const channelInput = (req.query.channel || req.query.channelId || '').trim();
  const maxResults = Number(req.query.maxResults || 3);
  if (!channelInput) return sendJSON(res, 400, { error: 'Yêu cầu không hợp lệ: thiếu tham số channel' });
  ...
  try {
    ...
    // 2) Lấy uploads playlist + tên kênh
